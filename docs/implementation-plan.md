...
#### 3.1 Claude Promptとの統合改善
- [x] プロンプトウィンドウの幅を50%に縮小
- [x] マネージャーのアクティブインスタンスへの送信対応
- [x] フォールバック処理（マネージャー → claude-cli）
- [ ] アクティブインスタンスの視覚的表示

#### 3.2 セッション名の重複問題対応（完了）
- [x] プロセスIDベースのセッション名（claude_PID_N形式）
- [x] 異なるNeovimプロセス間でのセッション名衝突を防止
- [x] 既存セッションの保護（削除せずに新規番号を割り当て）
- [x] 各プロセスで独立した番号付け（1から開始）
